...
void CTestClientDlg::OnBnClickedConnect()
{
	...
	if(socket.Connect(_T("localhost"), 3737) == FALSE) {
		return;
	}

	MPACKET packet;
	
	if(socket.Receive(&packet, sizeof(MPACKET))) {
		CString str;
		CString tmp;
		tmp.Format(_T("x : %d y : %d\r\n"), packet.xCoord, packet.yCoord);
		GetDlgItemText(IDC_DATA, str);
		str += tmp;

		SetDlgItemText(IDC_DATA, str);

		socket.Send(&packet, sizeof(MPACKET));
	}
}
